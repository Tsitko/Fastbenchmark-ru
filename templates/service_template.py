...
if str(target_col_name) + '_bot.py' not in os.listdir():
    print('If you want to provide token for your telegram bot, print it here, if not - just push enter:\n')
    token = input()
...
class bot_generator(object):
    def __init__(self, structure, enc_log):
        ...
        self.bot_code = []
        if str(self.target) + '_service_documentation.md' in os.listdir():
            if str(self.target) + '_bot.py' not in os.listdir():
                with open(str(self.target) + '_service_documentation.md') as file:
                    self.bot_code = file.read()
    ...
    def make_bot(self):
        if len(self.bot_code) > 0:
            ...
            if str(self.target) + '_bot.py' not in os.listdir():
                with open(str(self.target) + '_bot.py', 'w') as file:
...
class ModelFitter(object):
    ...
    def __init__(self):
        # reading structure
        try:
            with open(str(target_col_name) + '_structure.json', 'r') as file:
        ...
        try:
            self.final_model = pickle.load(open(str(target_col_name) + '_predictcion_final_model.dat', 'rb'))
        except:
        ...
        try:
            ...
            print('Csv with data set in not in path')
            quit()
