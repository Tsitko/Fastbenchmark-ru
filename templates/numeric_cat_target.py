...
if col_name in test.columns:
    ...
    train[col_name] = pd.to_numeric(train[col_name])
    test[col_name] = pd.to_numeric(test[col_name])
    train[col_name][pd.isna(train[col_name])] = np.nanmean(train[col_name])
    test[col_name][pd.isna(test[col_name])] = np.nanmean(train[col_name])
